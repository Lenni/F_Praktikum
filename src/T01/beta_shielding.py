...
def deine_mum(xs, ys, xerr, yerr, name, xlabel):
...
deine_mum(thickness, cnt - rausch_cnt, 0.01 * np.ones(len(thickness)), np.sqrt(cnt + rausch_cnt), "attenuation coefficient", "Distance /cm" )
deine_mum(dens, cnt - rausch_cnt, 0.01 * np.ones(len(dens)), np.sqrt(cnt + rausch_cnt), "mass attenuation coefficient", "Area Mass Density $\\frac{mg}{mm^2}$" )
