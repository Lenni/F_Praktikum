...
for file_n, title, vline_pos, ax in zip(["data/LAB/Schmitt/schmitt_{}.txt".format(a) for a in ["ein", "aus"]],
    ...
    if ax is ein_ax:
        ax.plot(v_ein, v_aus)
        ax.axvline(vline_pos - 0.05, label="Schaltpunk bei {}".format(vline_pos), alpha=0.5)
    else:
        ax.plot(-1.0 * v_ein + np.max(v_ein), v_aus)
        ax.set_xticks([0,2,4,6,8,10])
        ax.set_xticklabels([10, 8, 6, 4, 2, 0])
        ax.axvline(np.max(v_ein)-(vline_pos - 0.05), label="Schaltpunk bei {}".format(vline_pos), alpha=0.5)
    ax.set_xlabel("Eingangsspannung /V")
    ax.set_ylabel("Ausgangsspannung /V")
    ax.set_title(title)
    ax.axhline(10.672, label="10.622 V, Hohes Niveau", color='r', alpha=0.3)
    ax.axhline(1.519, label="1.569 V, Teifes Niveau", color='y', alpha=0.3)
