...
print("hier wird als 0 alles keiner als 0.1 betrachtet, dass ist sinnvoll wegen der Schwingungen bei den Nullbereichen des Ausgangssignal")
print("Hier treten massive Systematiken auf. Diese könnten von den Kapazitäten verursacht werden, die schon bei dem Kondensator aufgetreten.")

print("Für den Fit wird die Frequenz des Sinus separat bestimmt und dannach festgehalten.")
print("als erstes wird allerdings wird an am Sinus noch +c anggefittet")
#
# get Frequency and phase to determine the actual error
#
zero_vals = np.array(list(map(lambda x: x[1], filter(lambda x: x[0] <= 0.2, zip(u1, u2)))))

vol_sys_err = np.sum(zero_vals) / len(zero_vals)
vol_stat_err = np.sum((zero_vals - vol_sys_err)**2)/len(zero_vals) * np.ones(len(u1))
...
while True:
    ...
    my = np.sum(t)/len(t)
    sigma = np.sqrt(np.sum((t - my)**2) / (len(t)))
    maxis.append(my)
...
def simple_sine_func(x, amp, phase, c):
    return np.sin(x*freq  + phase) * amp +c
#
#fitte den Sinus an
#
#
#
sopt, scov, schi_sq = regression(simple_sine_func, time, u1, vol_stat_err, beta0=[2.0, 1.0, 0.0])
...
ropt, rcov, rchi_sq = regression(sine_func, fil_time, fil_u2, vol_stat_err[0] * np.ones(len(fil_time)),  beta0=[2.0/1.2, 1.0, 0.0])
print("""Eingangsspannungssinus
Amplitude\t: {:.3e} \pm {:.3e} Volt
Phase\t\t: {:.3e} \pm {:.3e} eins
c\t\t: {:.3e} \pm {:.3e} / V
