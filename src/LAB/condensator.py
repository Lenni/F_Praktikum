...
def do_eval(dataFile, name, many, epsilon, func):
    ...
    opt, cov, chi_sq = regression(func, times, voltages, voltage_err)
    i = 0
    while 2 * (i) + 1< len(opt):
    ...
    simple_figure(times, None, voltages, voltage_err, func(times, *opt), name,
        "Zeit /s", "Kondensatorspannung /V", "protocols/LAB/Plots/Capacitor/Capacitor{}.png".format(name))
    print("\n\n\n")
do_eval(discharge_file, "Entladung", lambda x, tau, a, tau2, a2, tau3, a3, tau4, a4: sum(map(lambda a: a[1]*np.exp(-1.0 * x / a[0]), take_pairs(list(locals().values())[1:]))))
...
do_eval(charge_file, "Aufladung", lambda x, tau, a, tau2, a2: a * (1-np.exp(-1.0* x /tau)) + a2 * (1-np.exp(-1.0* x /tau2)))
