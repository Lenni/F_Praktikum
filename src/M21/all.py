...
pos_td_err = 1/np.sqrt(3) * 0.1

mitte = 0.5 * (pos_td[0] + pos_td[1])
mitte_err = 0.5 * np.sqrt(2) * pos_td_err

motor_pos_lin = weg_m * motor_pos + weg_c - mitte
motor_pos_lin_err = np.sqrt(np.sum(np.array([ err ** 2 for err in [motor_pos * weg_m_err, 
        weg_c_err * np.ones(len(motor_pos)), mitte_err] ])))

delta_l = 3*10**-2 * td_mittelwert
deltal_err =  (3*10**-2 * td_error)* np.ones(len(motor_pos))
